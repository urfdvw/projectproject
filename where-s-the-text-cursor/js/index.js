...
const showCustomUI = e => {
  ...
  const toggleItem = (dir = 'next') => {
    ...
    if (!input.__SELECTED_ITEM) {
      ...
      if (!nextActive && dir === 'next') nextActive = list.firstChild; else
        if (!nextActive) nextActive = list.lastChild;
  ...
  const filterList = () => {
    ...
    if (input.__CUSTOM_UI.firstChild)
      input.__CUSTOM_UI.replaceChild(suggestedList, input.__CUSTOM_UI.firstChild); else
